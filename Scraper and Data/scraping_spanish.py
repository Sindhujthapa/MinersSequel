...
def fetch_reviews_for_app(app_id, target_reviews=500):
    all_reviews = []
    seen_ids = set()  # Track seen recommendation IDs
    
    # We'll try different filtering combinations to get more reviews
    ...
    while len(all_reviews) < target_reviews:
        # Prepare current params
        current_params = params.copy()
        current_params["cursor"] = cursor
        param_key = f"{current_params['filter']}_{current_params['review_type']}_{current_params['purchase_type']}"
        
        # If we've had too many empty pages, try new parameters
        if consecutive_empty_pages >= max_consecutive_empty:
            # Try a new filter combination if we haven't tried all options
        ...
        try:
            ...
            # Track number of new reviews in this batch
            new_reviews_count = 0
            ...
            if not raw_cursor:
                ...
                consecutive_empty_pages = max_consecutive_empty  # Force parameter change
                continue
        ...
        # Additional exit condition for when we've tried multiple approaches but can't reach target
...
def create_balanced_dataset(df):
    ...
    # Sample from positive reviews to match number of negative reviews
...
def main():
    ...
    for i, app_id in enumerate(APP_IDS):
        ...
        # Save interim results every few games to prevent total data loss if script crashes
    ...
    for review in combined_data:
        rec_id = review[0]  # recommendationid is the first element
    ...
    # Create balanced dataset with 50-50 split
